...
| 本番カード登録・本番課金につながる導線を出さない | 必須 | `customer/register.html` の未送信カード欄削除済み。`customer/payment.html` は実決済禁止として再確認 |
...
| 決済関連画面が準備中・誤認防止表示になっている | 必須 | `customer/payment.html`, `worker/rewards.html`, `admin/payments.html` を確認 |
