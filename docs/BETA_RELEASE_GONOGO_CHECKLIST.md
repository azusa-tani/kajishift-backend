...
| 問い合わせ・事前登録・β利用希望受付として利用できる | OK（一部継続確認） | 2026-07-03に customer/worker 問い合わせ送信、admin dashboard / admin support での受付確認を本番URLで確認済み。事前登録・β利用希望受付の残り導線は別途実ブラウザ確認を継続 |
...
## 2026-07-07 A案 Conditional Go 最終整理

判定は **Conditional Go** とする。確認者は `KAJISHIFT運用担当`。この記録には、個人名、メールアドレス、住所、電話番号、問い合わせ本文、審査回答本文、問い合わせID、userId風の値、workerId、bookingId、予約ID、submissionId、APIキー、Secret、DB接続文字列、決済情報の実値を記録しない。

理由:

- 問い合わせ、事前登録、β利用希望受付、サービス内容閲覧については、誤認防止文言と本番決済/正式予約導線の抑止が本番反映済み。
- Frontend `d1f1de1` はVercel Productionでsuccess、Backend docs `ae2bcd4` も反映済み。
- 管理画面更新系、実ユーザーtokenでの最終目視、`customer/select-worker.html` 認証済み本体確認、`worker/job-detail.html` 403は残るため、公開範囲限定・更新系操作禁止を条件とする。

A案で公開してよい範囲:

- トップページ / LP / サービス紹介、利用の流れ、事前登録画面、β利用希望導線、問い合わせ導線。
- 依頼者側の読み取り中心画面、ワーカー事前登録・案内系画面、本番決済・正式予約・カード登録未開始である注記付きの説明ページ。
- 問い合わせ、事前登録、β利用希望、サービス内容・対応範囲の閲覧、運営からの個別案内前提の相談。

A案で公開しない範囲:

- 正式予約、本番決済、本番カード登録、本番課金につながる導線、予約確定操作。
- worker受注・辞退・作業完了。
- 管理画面の承認/却下/更新/削除/CSV出力などの更新系操作。
- Stripe本番Webhook・本番決済運用。

Conditional Goの条件:

- 公開範囲は問い合わせ・事前登録・β利用希望受付・サービス内容閲覧に限定する。
- 一般公開時の案内文言は `β版受付中`、`事前登録`、`β利用希望`、`お問い合わせ` を中心にする。
- `今すぐ予約`、`予約確定`、`決済`、`カード登録`、`本日から利用開始` と見える案内は使わない。
- 公開直前に運用担当が本番URLを読み取り限定で確認する。
- 登録送信、予約作成、予約確定、決済、カード登録、承諾、辞退、作業完了、管理更新系操作は行わない。
- 問い合わせ・β利用希望が来ても、正式予約として扱わず、運営からの個別案内対象とする。

公開前の最終手動確認:

- トップ / flow / customer登録 / worker登録に `β版受付中`、`事前登録`、`β利用希望`、`お問い合わせ` が出ていること。
- 本番決済・正式予約・カード登録未開始の注記が見えること。
- `customer/dashboard.html`、`customer/bookings.html`、`customer/booking-detail.html` に `予約確定` が残っていないこと。
- `β確認済み（正式予約は未開始）` が表示されること。
- ダッシュボード通知がA案向け汎用文言になっていること。
- Stripe.js / カード入力UI / 決済ボタンが出ないこと。
- `/payments/intent` が発生しないこと。
- NetworkでDB更新・決済系のPOST/PUT/PATCH/DELETEが出ないこと。
- Console重大エラーがないこと。
- 確認中に登録送信、予約作成、予約確定、決済、カード登録、承諾、辞退、作業完了、管理更新ボタンを押さないこと。

A案公開後の注意事項:

- 問い合わせとβ利用希望は、正式予約ではなく「運営確認後の個別案内」として対応する。
- ユーザーには、本番決済・正式予約・カード登録は未開始であることを明示する。
- β利用希望者には、利用可否・開始時期・対象範囲を個別連絡で案内する。
- Slack通知や問い合わせ一覧を定期確認する。
- 管理画面では更新・削除・承認/却下などを不用意に押さない。
- 既存docsには過去証跡の実値が残っている可能性があるため、共有範囲が広がる前にdocsサニタイズを別タスク化する。
...
## 2026-07-03 A案問い合わせ導線確認

| 画面 | 判定 | 確認結果 |
|------|------|----------|
| `customer/support.html` | OK | 本番URLで表示確認済み。依頼者ログイン後、問い合わせフォームが正常表示され、件名、問い合わせ種別、返信先メールアドレス、本文を入力できた。テスト問い合わせを1件送信し、受付完了メッセージを確認。`POST /api/support` は201、`/api/auth/me`, `/api/public/status`, `/api/notifications/unread-count` は確認範囲で200。Console重大エラーなし、継続的な 500 / 404 / 429 / CORS なし |
| `worker/support.html` | OK | 本番URLで表示確認済み。ワーカーログイン後、問い合わせフォームが正常表示され、件名、問い合わせ種別、返信先メールアドレス、本文を入力できた。テスト問い合わせを1件送信し、受付完了メッセージを確認。`POST /api/support` は201、`/api/public/status` 系は確認範囲で200。Console重大エラーなし、継続的な 500 / 404 / 429 / CORS なし |
| `admin/dashboard.html` | OK | 本番URLで表示確認済み。customer/worker側から送信したテスト問い合わせが「未対応の問い合わせ」に表示され、「すべて見る」導線から問い合わせ管理へ進める状態を確認。「対応する」ボタンはDB更新を伴う可能性があるため未押下 |
| `admin/support.html` | OK | 本番URLで表示確認済み。問い合わせ一覧が正常表示され、customer/worker側から送信したテスト問い合わせを確認。ステータス「新規」、アサイン「未アサイン」を確認。`対応する`, `自分にアサイン`, `対応開始`, `削除` はDB更新・削除を伴う可能性があるため未押下。`GET /api/support?limit=1000`, `/api/auth/me`, `/api/public/status`, `/api/notifications/unread-count` は確認範囲で200。Console重大エラーなし、継続的な 500 / 404 / 429 / CORS なし |

上記により、A案「本番決済なし限定公開」として問い合わせ受付導線はOK扱いとする。確認中に本番決済、返金、カード登録、正式有料予約受付への導線は表示されていない。問い合わせ本文、メール、ID風の値、個人情報、住所、予約ID、決済情報の実値は記録しない。Stripe本番決済・本番Webhook確認は未実施であり、B案移行前の必須残タスクとして残す。
