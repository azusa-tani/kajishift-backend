...
β版では、未連携UIや固定サンプルが実運用設定・実データに見えないように非表示または準備中表示へ整理済み。運用時は以下を「画面から設定・確認できる機能」として扱わない。

| 項目 | β運用での扱い |
|------|---------------|
| 報酬/精算詳細、振込状態、自動精算 | 画面上の固定表示は削除済み。β中は運営が個別確認・個別案内する |
| 返金、キャンセル料、決済/売上/報酬精算の本格管理 | Stripe Dashboard、CSV、運用記録で代替。アプリ内の本格管理画面は準備中 |
| 日別売上推移グラフ | `admin/dashboard.html` では準備中表示。KPIカードのみ実API由来として扱う |
| メールテンプレート、プッシュ通知、問い合わせ連絡先編集 | `admin/settings.html` では準備中表示。変更が必要な場合はコード・環境設定・運用担当者確認で対応する |
| 操作ログ検索・CSV出力 | 固定サンプルは削除済み。必要な操作確認はサーバーログ、DB監査対象、管理者間の運用記録で確認する |
| LINEログイン | β版では未提供。通常のメール/パスワードログインを案内する |
